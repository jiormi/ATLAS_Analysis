...
double returnEfold(double Etrue = 40, double sigMC = 4.0, double s = 0.98, double r = 1.4) {
    double Ereco = gRandom->Gaus(Etrue, sigMC);
    double Efold = s*Ereco + (r-s)*(Ereco-Etrue);
...
void testFF() {
  ...
  TFile *f = TFile::Open("FFtest.root","RECREATE");
  ...
  for(int bin=1;bin<ptBinsNum;bin++){
    ...
    for(auto s_i:s_values){
      for(auto r_i:r_values){
        ...
        for(int event = 0; event<Nevents_int; event++){
        double EfoldedDataMC = returnEfold((low + up) / 2. , sigMC, s_i,r_i);
